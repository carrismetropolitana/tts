...
export async function runnerPatterns() {
	...
	for (const [lineIndex, lineData] of allLinesData.entries()) {
		...
		for (const [patternIndex, patternId] of lineData.patterns.entries()) {
			...
			const patternGroup: Pattern[] = await patternResponse.json();
			...
			if (ttsHasChanged) {
				...
				LOGGER.info(`[${lineIndex}/${allLinesData.length}] [${patternIndex}/${lineData.patterns.length}] Generated | Line ${lineData.id} | Pattern ${patternData.id} | ${patternTts}`);
			}
